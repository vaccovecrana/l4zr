...
public class L4Ps extends L4St implements PreparedStatement {
  ...
  private void executeInternal() throws SQLException {
    ...
    try {
      ...
      var response = isSelect ? client.query(statement) : client.execute(statement);
      ...
      if (resultSetAvailable) {
        ...
        if (closeOnCompletion) {
          isClosed = true; // Close statement after ResultSet is created
        }
      }
    } catch (Exception e) {
      throw badExec(e);
  ...
  @Override public ResultSetMetaData getMetaData() throws SQLException {
    checkClosed();
    try {
      var response = client.query(statement);
      if (response.results.isEmpty() || response.results.get(0).isError()) {
        throw generalError("Unable to retrieve metadata");
      }
      return new L4RsMeta(response.results.get(0));
    } catch (Exception e) {
      throw badQuery(e);
    }
  }
